<script lang="ts">
	...
	let {
		title = 'The Lebanon Displacement Diaries',
		message = 'Check out The Lebanon Displacement Diaries',
		url,
		class: className = '',
		iconClass = 'text-lebblack hover:text-burgundy',
	...
	let siteUrl = $state('');
	const encodedUrl = $derived(encodeURIComponent(siteUrl));
	const encodedMessage = $derived(encodeURIComponent(message));
	...
	const whatsappUrl = $derived(
		`https://wa.me/?text=${encodedMessage}%20by%20The%20New%20Humanitarian:%20${encodedUrl}`
	);
	const emailUrl = $derived(
		`mailto:?subject=${encodedTitle}&body=${encodedMessage}%20by%20The%20New%20Humanitarian:%20${encodedUrl}`
	);
	const blueskyShareUrl = $derived(
		`https://bsky.app/intent/compose?text=${encodedMessage}%20by%20@newhumanitarian.bsky.social:%0A${encodedUrl}%0A%0Ahttps://bsky.app/profile/newhumanitarian.bsky.social`
	);

	onMount(() => {
		showIcons = initiallyExpanded;
		if (!browser) return;

		siteUrl = (url ?? window.location.href).replace(
			'https://localhost:5173',
			'https://interactive.thenewhumanitarian.org'
		);
	});
</script>

<div class={`share-icons flex items-center justify-center gap-x-2 ${className}`}>
	...
	<div class={`flex items-center gap-1 sm:gap-2 ${showIcons ? 'block' : 'hidden'} sm:flex`}>
		...
		<!-- Email -->
		<a
			class={`mx-auto w-full ${iconClass}`}
			href={emailUrl}
			target="_blank"
			rel="noopener noreferrer"
