...
!(function (a) {
	...
	function b() {}
	function c(a, b, c, d) {
		'addEventListener' in window ? a.addEventListener(b, c, !!Ia && (d || {})) : 'attachEvent' in window && a.attachEvent('on' + b, c)
	}
	function d(a, b, c) {
	...
	function f(a) {
		var b,
			...
			f = 0,
			g = function () {
				;(f = La()), (e = null), (d = a.apply(b, c)), e || (b = c = null)
			}
		return function () {
			var h = La()
			f || (f = h)
			...
			return (
				(b = this),
				(c = arguments),
				i <= 0 || i > ya ? (e && (clearTimeout(e), (e = null)), (f = h), (d = a.apply(b, c)), e || (b = c = null)) : e || (e = setTimeout(g, i)),
				d
			)
		}
	}
	...
	function h(a) {
		ma && 'object' == typeof window.console && console.log(g(a))
	}
	function i(a) {
		'object' == typeof window.console && console.warn(g(a))
	}
	function j() {
		k(),
	...
	function k() {
		...
		;(pa = c[0]),
			(Y = a !== c[1] ? Number(c[1]) : Y),
			(aa = a !== c[2] ? b(c[2]) : aa),
			(ma = a !== c[3] ? b(c[3]) : ma),
			(ka = a !== c[4] ? Number(c[4]) : ka),
			(V = a !== c[6] ? b(c[6]) : V),
			(Z = c[7]),
			(ga = a !== c[8] ? c[8] : ga),
			(X = c[9]),
			(_ = c[10]),
			(va = a !== c[11] ? Number(c[11]) : va),
			(ja.enable = a !== c[12] && b(c[12])),
			(ra = a !== c[13] ? c[13] : ra),
			(Ba = a !== c[14] ? c[14] : Ba)
	}
	function l() {
		function a() {
			...
			h('Reading data from page: ' + JSON.stringify(a)),
				(Da = 'messageCallback' in a ? a.messageCallback : Da),
				(Ea = 'readyCallback' in a ? a.readyCallback : Ea),
				(ua = 'targetOrigin' in a ? a.targetOrigin : ua),
				(ga = 'heightCalculationMethod' in a ? a.heightCalculationMethod : ga),
				(Ba = 'widthCalculationMethod' in a ? a.widthCalculationMethod : Ba)
		}
		function b(a, b) {
			return 'function' == typeof a && (h('Setup custom ' + b + 'CalcMethod'), (Ga[b] = a), (a = 'custom')), a
		}
		'iFrameResizer' in window && Object === window.iFrameResizer.constructor && (a(), (ga = b(ga, 'height')), (Ba = b(Ba, 'width'))),
	...
	function q(a) {
		...
		a.eventNames && Array.prototype.map ? ((a.eventName = a.eventNames[0]), a.eventNames.map(b[a.method])) : b[a.method](a.eventName),
			h(e(a.method) + ' event listener: ' + a.eventType)
	}
	function r(a) {
	...
	function C() {
		...
		function l() {
			Array.prototype.forEach && document.querySelectorAll
				? (h('Setting up location.hash handlers'), g(), j(), k())
				: i('In page linking not fully supported in this browser! (See README.md for IE8 workaround)')
		}
		return ja.enable ? l() : h('In page linking not enabled'), { findTarget: e }
	}
	function D() {
		h('Enable public methods'),
			(Ca.parentIFrame = {
				...
				size: function (a, b) {
					O('size', 'parentIFrame.size(' + (a || '') + (b ? ',' + b : '') + ')', a, b)
				},
			})
	}
	function E() {
	...
	function H(a, b) {
		function c(a) {
			if (/^\d+(px)?$/i.test(a)) return parseInt(a, W)
			var c = b.style.left,
	...
	function J(a, b) {
		for (var c = b.length, d = 0, f = 0, g = e(a), i = La(), j = 0; j < c; j++) (d = b[j].getBoundingClientRect()[a] + H('margin' + g, b[j])) > f && (f = d)
		return (i = La() - i), h('Parsed ' + c + ' HTML elements'), h('Element position calculated in ' + i + 'ms'), I(i), f
	}
	function K(a) {
	...
	function N(b, c, d, e) {
		...
		function g() {
			function b(a, b) {
				return !(Math.abs(a - b) <= va)
			}
			return (m = a !== d ? d : Ma[ga]()), (n = a !== e ? e : Na[Ba]()), b(ea, m) || (aa && b(za, n))
		}
		function i() {
	...
	function O(a, b, c, d) {
		...
		f() ? h('Trigger event cancelled: ' + a) : (e(), 'init' === a ? N(a, b, c, d) : Oa(a, b, c, d))
	}
	function P() {
	...
	function T(a) {
		...
		function e() {
			return (
				(!('undefined' != typeof module && module.exports) && 'iFrameResize' in window) || ('jQuery' in window && 'iFrameResize' in window.jQuery.prototype)
			)
		}
		function f() {
	...
	if ('undefined' != typeof window) {
		...
		var La =
			...
			Ma = {
				...
				max: function () {
					return Math.max.apply(null, K(Ma))
				},
				min: function () {
					return Math.min.apply(null, K(Ma))
				},
				grow: function () {
					return Ma.max()
				},
				lowestElement: function () {
					return Math.max(Ma.bodyOffset() || Ma.documentElementOffset(), J('bottom', M()))
				},
			...
			Na = {
				...
				scroll: function () {
					return Math.max(Na.bodyScroll(), Na.documentElementScroll())
				},
				max: function () {
