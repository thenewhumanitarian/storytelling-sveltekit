...
async function fetchAllProjects() {
	...
	const results = await Promise.all(PROJECTS.map((project) => fetchProjectData(project)));
...
async function main() {
	try {
		...
		if (requestedProjects.length > 0) {
			...
			console.log(
				`🎯 Running filtered projects: ${filteredProjects.map((p) => p.name).join(', ')}`
			);
...
const isMainModule =
	import.meta.url === `file://${process.argv[1]}` || process.argv[1]?.includes('pre-fetch-data.js');
